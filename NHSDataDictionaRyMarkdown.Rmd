...
football_xpath <- '//*[@id="u7035812909699189"]/div/div[2]/div/div/div[2]/div/div/table'
...
c19_reduced <- covid19 %>% 
  select(TotalCases, TotalRecovered, TotalDeaths, TotalTests, `Country,Other`, Continent)
...
```{r data_prep}
c19_numerics <- c19_reduced[,1:4] %>% 
  suppressWarnings(purrr::map_dfr(~ as.numeric(gsub(",", "", .)))) %>% 
  #Fill in NAs if they are zero
  purrr::map_dfr(~ ifelse(is.na(.),0, .))

```

We then ***purrr::map_dfr*** over the data frame to convert to numeric and strip out the commas and then to fill in the NA values with a zero if they are missing. Finally, I will join the data back on to the original frame to bring back in the country:
